...
from conpot.core.loggers.logstash import Logstash
from conpot.core.loggers.mongodb_log import MongodbLogger
...
class LogWorker(object):
    def __init__(self, config, dom, session_manager, public_ip):
        ...
        if config.getboolean('mongodb', 'enabled'):
            ...
            sensorid = config.get('common', 'sensorid')
            self.mongodb_logger = MongodbLogger(host, port, db, collection, sensorid)
