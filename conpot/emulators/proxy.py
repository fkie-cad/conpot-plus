...
class ProxyAddresses(object):
    orig_addrs = {}

    @staticmethod
    def put(proxy_sock, orig_addr):
        proxy_addr_str = ':'.join([str(x) for x in proxy_sock.getsockname()])
        ProxyAddresses.orig_addrs[proxy_addr_str] = orig_addr

    @staticmethod
    def get(proxy_addr):
        proxy_addr_str = proxy_addr[0] + ':' + str(proxy_addr[1])
        if proxy_addr_str in ProxyAddresses.orig_addrs:
            orig_addr = ProxyAddresses.orig_addrs[proxy_addr_str]
            return orig_addr
        return None

    @staticmethod
    def try_replace_addr(addr):
        proxy_addr = ProxyAddresses.get(addr)
        return proxy_addr if proxy_addr is not None else addr

    @staticmethod
    def clear_unused(proxy_sock):
        proxy_addr_str = ':'.join([str(x) for x in proxy_sock.getsockname()])
        ProxyAddresses.orig_addrs.pop(proxy_addr_str, None)

    @staticmethod
    def clear_all():
        ProxyAddresses.orig_addrs = {}
